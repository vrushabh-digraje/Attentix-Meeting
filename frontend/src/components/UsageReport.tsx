...
const UsageReport: React.FC<UsageReportProps> = ({ user, meeting, onReturnToMeeting }) => {
    ...
    return (
        <div className="bg-brandBg text-slate-100 min-h-screen flex flex-col font-sans">
            
            {/* Navigation header */}
            <nav className="flex justify-between items-center px-8 py-4 border-b border-white/5 bg-[#18181a] backdrop-blur-md sticky top-0 z-50">
                ...
                <div className="flex items-center gap-4">
                    ...
                    <span className="bg-white/5 border border-white/10 px-4 py-1.5 rounded-full text-xs text-slate-300 font-medium">
                        Host
                    </span>
                </div>
            </nav>
            ...
            <div className="max-w-7xl mx-auto py-10 px-6 w-full flex-grow flex flex-col gap-8">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-4 gap-8 items-start">
                    ...
                    <div className="lg:col-span-1 bg-zoomPanel border border-white/5 p-6 rounded-2xl flex flex-col gap-4 lg:sticky lg:top-24 max-h-[calc(100vh-140px)] overflow-y-auto shadow-lg">
                        <h3 className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2 flex items-center gap-2 pb-3 border-b border-white/5">
                            <Users size={12} /> Live Scoreboard
                        </h3>
                        
                        <div className="flex flex-col gap-3">
                            {activeList.length === 0 ? (
                                <span className="text-slate-500 text-xs">No active participants.</span>
                            ) : (
                                activeList.map(p => {
                                    ...
                                    return (
                                        <div key={p.user_id} className="flex justify-between items-center bg-white/5 border border-white/5 px-4 py-2.5 rounded-xl text-xs font-semibold">
                                            <span className="text-slate-300 font-bold">{p.username}</span>
                    ...
                    <div className="lg:col-span-3 flex flex-col gap-8">
                        ...
                        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6">
                            <div>
                                <h2 className="text-2xl font-extrabold tracking-tight">Meeting Room</h2>
                                <p className="text-slate-400 text-xs mt-1">Classroom engagement metrics & continuous attention analysis.</p>
                            </div>
                            
                            <div className="flex gap-4 w-full sm:w-auto">
                                <div className="glass-panel px-5 py-3 rounded-xl flex-grow sm:flex-grow-0 text-center min-w-[120px]">
                                    <span className="text-[9px] font-semibold text-slate-400 uppercase tracking-wider block">Active Users</span>
                                    <h3 className="text-2xl font-extrabold text-zoomBlue mt-0.5">{activeList.length}</h3>
                                </div>
                                <div className="glass-panel px-5 py-3 rounded-xl flex-grow sm:flex-grow-0 text-center min-w-[120px]">
                                    <span className="text-[9px] font-semibold text-slate-400 uppercase tracking-wider block">Average Attention</span>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div className="glass-panel p-6 rounded-2xl flex flex-col justify-between h-[300px]">
                                <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Class Attention Trend</h3>
                                <div className="relative flex-grow h-[200px]">
                                    <canvas ref={trendCanvasRef}></canvas>
                                </div>
                            </div>
                            <div className="glass-panel p-6 rounded-2xl flex flex-col justify-between h-[300px]">
                                <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">State Distribution</h3>
                        ...
                        <div>
                            <h3 className="text-sm font-bold text-slate-400 uppercase tracking-wider mb-4">Detailed Participant Matrix</h3>
                            {activeList.length === 0 ? (
                                <div className="glass-panel py-12 rounded-2xl text-center text-xs text-slate-500">
                                    ...
                                    {activeList.map(p => {
                                        ...
                                        return (
                                            <div key={p.user_id} className={`glass-panel p-5 rounded-xl border-2 ${borderClass} relative flex flex-col items-center text-center transition-all duration-300`}>
                                                ...
                                                <div className="w-12 h-12 rounded-full bg-gradient-to-r from-cyanAccent to-purpleAccent flex items-center justify-center text-white font-extrabold text-lg mb-3 shadow-md">
                                                    {p.username.charAt(0).toUpperCase()}
                                                </div>
                                                <h4 className="font-bold text-sm text-slate-100">{p.username}</h4>
                                                <p className="text-[10px] text-slate-400 mt-0.5">State: <strong className={scoreColor}>{p.state}</strong></p>
                                                <div className={`text-2xl font-black ${scoreColor} mt-2.5`}>{p.score}%</div>
                                                <p className="text-[9px] text-slate-500 mt-0.5">Warnings: {p.warnings}/3</p>
                                                <button 
                        ...
                        <div className="glass-panel p-6 rounded-2xl">
                            <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-4">Distraction Logs</h3>
                            
                            <div className="overflow-x-auto w-full">
                                <table className="w-full text-left border-collapse">
                                    <thead>
                                        <tr className="border-b border-white/5 text-[10px] font-semibold text-slate-400 uppercase tracking-wider">
                                    ...
                                    <tbody className="divide-y divide-white/5 text-xs">
                                        {logs.length === 0 ? (
                                            <tr>
                                                <td colSpan={5} className="py-6 text-center text-slate-500 text-[11px]">No focus alert events logged.</td>
                                            </tr>
                                        ) : (
                                            logs.map((log, idx) => {
                                                ...
                                                return (
                                                    <tr key={idx} className="hover:bg-white/5 transition-all">
                                                        ...
                                                        <td className="py-2.5 px-4 font-bold text-slate-200">{log.score}%</td>
                                                        <td className="py-2.5 px-4 text-slate-400">{log.warnings}/3</td>
                                                        <td className="py-2.5 px-4 text-slate-500 font-mono text-[10px]">{log.timestamp}</td>
                                                    </tr>
                                                );
                                            })
                                        )}
            ...
            {criticalAlertStudent && (
                <div className="fixed inset-0 bg-black/85 backdrop-blur-sm z-50 flex items-center justify-center">
                    <div className="glass-panel p-8 rounded-2xl border border-stateRed shadow-2xl shadow-stateRed/25 max-w-sm w-full text-center mx-4">
                        ...
                        <p className="text-slate-300 leading-relaxed text-xs mb-6">
